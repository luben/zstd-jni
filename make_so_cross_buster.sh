...
compile() {
    ...
    ssh buster 'cd '$BUILD_DIR'; '$CC' -shared -static-libgcc -Wl,--version-script=./libzstd-jni.so.map -Wl,-Bsymbolic -Wl,-z,relro,-z,now -fPIC -O3 -flto -DZSTD_MULTITHREAD=1 -I/usr/include -I./jni -I./native -I./native/common -std=c99 -lpthread -o libzstd-jni-'$VERSION'.so native/*.c native/common/*.c native/compress/*.c native/decompress/*.c native/dictBuilder/*.c'
