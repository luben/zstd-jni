...
class ZstdSpec extends FlatSpec with Checkers {
  ...
  // -1 == disable custom bytes
  // Int.MaxValue == recommended size
  val bufferSizes = List(-1, 2048, Int.MaxValue)
    ...
    for (bufferSize <- bufferSizes) {
      "ZstdInputStream" should s"should round-trip compression/decompression at level $level and buffersize $bufferSize" in {
        check { input: Array[Byte] =>
          ...
          val zos = bufferSize match {
            case -1 =>
              new ZstdOutputStream(os, level)
            case Int.MaxValue =>
              new ZstdOutputStream(os, level, new Array[Byte](ZstdOutputStream.dstSize))
            case _ =>
              new ZstdOutputStream(os, level, new Array[Byte](bufferSize))
          }
          val block = 128 * 1024
          var ptr = 0
          while (ptr < size) {
          ...
          val zis = bufferSize match {
            case -1 =>
              new ZstdInputStream(is)
            case Int.MaxValue =>
              new ZstdInputStream(is, new Array[Byte](ZstdInputStream.srcBuffSize))
            case _ =>
              new ZstdInputStream(is, new Array[Byte](bufferSize))
          }
